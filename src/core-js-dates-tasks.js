...
function getNextFriday(date) {
  ...
  if (dayOfWeek === 5) {
    ...
    dayToFriday = (7 + 5 - dayOfWeek) % 7;
  }
  date.setDate(date.getDate() + dayToFriday);
  return date;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const countDays = (end.getTime() - start.getTime()) / day;
  return countDays + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return start <= dateMiliseconds && end >= dateMiliseconds;
}
...
function formatDate(date) {
  ...
  return `${dMonth}/${dDay}/${dYear}, ${hours}:${minutes}:${seconds} ${AmPm}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  date2.setFullYear(year, month, 0);

  let totalWeekend = 0;

  for (let index = date1.getDate(); index <= date2.getDate(); index += 1) {
    date1.setDate(index);
...
function getWeekNumberByDate(date) {
  ...
  const day = 24 * 3600 * 1000;
  const days = Math.floor((date - startDate) / day);
...
function getNextFridayThe13th(date) {
  ...
  while (condition) {
    for (let index = month; index < 12; index += 1) {
      const dates = new Date(year, index, 13);
      if (dates.getDay() === 5) {
        return dates;
      }
    }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const startDateDay = startDate.getTime();
  const endDateDay = endDate.getTime();
  const countDays = (endDateDay - startDateDay) / day + 1;

  const arr = [];
  for (let index = 0; index < countDays; index += 1) {
    const date = new Date(startDateDay + index * day);
    const [yyyy, mm, dd] = [
      date.getFullYear(),
  ...
  const workDays = arr.filter(
    (__, index) => index % (countWorkDays + countOffDays) < countWorkDays
  );
  return workDays;
}
...
function isLeapYear(date) {
  ...
  return year % 4 === 0 && year / 100 !== Number.isInteger(year);
}
